...
public class PositionEstimator {
  ...
  public PositionEstimator(double period) {
    ...
    m_peLoop.schedule(new PositionEstimatorTask(this), 0L,
        (long) (m_period * 1000));
  ...
  private void calculate() {
    ...
    synchronized (this) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        posEst[i] = lastPosEst[i] + lastVelEst[i] * deltaT
            + 0.5 * deltaT * deltaT * lastAccelEst[i];
      }
    ...
    if (setBestGearTarget()) {
      ...
      for (int i=0; i<2; i++) {
        // Only take 10% of the correction due to sensor error
         double delX = 0.1 * (newPosition[i] - lastPosEst[i]);
