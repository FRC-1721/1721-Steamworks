...
public class RobotMap {
  ...
  /** Encoder DIO Ports **/
  public static final int dtlEncPA = 0, dtlEncPB = 1, dtrEncPA = 2,
      dtrEncPB = 3;
   ...
   * 
   * 1. Check Directions of the motors and the encoders. left motor should move forward with a
   * positive value, and the left encoder should grow (+). right motor should move backwards with a
   * positive value, and the right encoder should grow (+). 2. Put robot in test mode in Driver
   * Station, move robot forward a few feet. 3. 'desired' is the number of feet (exactly) you moved
   * the robot 'actual' is the number you got 'current' is the current DPP values Use the formula
   * '(desired/actual)*current' distance pure pulse to calculate the new DPP
   * 
   */
  ...
  public static final double navRateP = 0.0, navRateI = 0.0, navRateD = 0.0,
      navRateF = 0.001;
  ...
  // Put in order of likelihood to get a hit for faster load times
  public static final String[] gamepads =
      {"Gamepad F310 (Controller)", "Controller (Gamepad F310)"};

  // Number of USB ports to scan
  /**
   * The Driver Station will now show up to 6 devices in the Setup window. The first 4 devices will
   * be transmitted to the robot. The additional devices are shown to allow teams to use one
   * component of a composite device, such as the TI Launchpad with FRC software without having to
   * sacrifice one of the 4 transmitted devices.
   */
  public static final int numUSB = 3;

  // TODO I might have to do something about device 5 and 6 if we do something with the custom
  // Driver Station.

  // Joystick to have PID buttons on, and the buttons to use
  public static final int pidStick = 0, // Note: This will crash if the Joystick doesn't exist, I
                                        // recommend only making it controller two (1) if you know
                                        // you're going to use tank drive
      pidDisableButton = 1, pidEnableButton = 8, forwardDriveButton = 6,
  ...
  // Update some parameters based on what has been setup
  // Disable p term if encoders are not working. Feedfoward term is essentially
  // manual mode
  static {
